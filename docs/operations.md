...
`tidal-mob-push-lens` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for push workflows, centered on event replay, fixture event logs, and golden state snapshots.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 2.
